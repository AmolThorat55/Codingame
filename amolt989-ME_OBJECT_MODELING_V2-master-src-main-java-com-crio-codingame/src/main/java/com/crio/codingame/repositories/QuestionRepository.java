...
public class QuestionRepository implements IQuestionRepository {
    ...
    public List<Question> findAll() {
        ...
        for(Map.Entry<String, Question> me : questionMap.entrySet()){
            list.add((Question) me.getValue());
        }
     return list;
    }
    ...
    public List<Question> findAllQuestionLevelWise(Level level) {
        List<Question> levelWiseQ = new ArrayList<>();
        List<Question> allQs = findAll();
        for(Question q : allQs){
            if(q.getLevel() == level) levelWiseQ.add(q);
        }
     return levelWiseQ;
    }

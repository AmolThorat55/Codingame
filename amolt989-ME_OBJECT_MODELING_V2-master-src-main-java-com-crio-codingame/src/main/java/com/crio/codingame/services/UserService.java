...
public class UserService implements IUserService {
    ...
    public User create(String name) {
        ...
        User user = new User(Integer.toString(1),name, 00, contests); 
    ...
    public List<User> getAllUserScoreOrderWise(ScoreOrder scoreOrder){ 
        ...
        if(scoreOrder == scoreOrder.ASC){
            return userList.stream().sorted( (u1, u2) -> u1.getScore().compareTo( u2.getScore() )).collect(Collectors.toList());
        }  
        return userList.stream().sorted( (u1, u2) -> u2.getScore().compareTo( u1.getScore() )).collect(Collectors.toList());
    ...
    UserRegistrationDto urd = null;
    if(user.checkIfContestExists(contest)){
        user.deleteContest(contest);
        userRepository.save(user);
        urd = new UserRegistrationDto(contest.getName(),user.getName(), RegisterationStatus.NOT_REGISTERED);
